...
if (target === 'backup') {
  ...
  for (const [browser, manifestFile] of Object.entries(targets)) {
    ...
    // Write clean browser zip into Releases/ directory (chrome.zip, firefox.zip, opera.zip)
    const releaseFile = path.join(releaseDir, `${browser}.zip`);
    zip.writeZip(releaseFile);

    console.log(`✓ Releases/${browser}.zip`);
  }
